...
from frontend.assets.UserInterface.PostUI.UserInterfaceFile.DetaliedPostView import Ui_DetaliedPostView
...
class DetailedPostWidget(QFrame):
    ...
    def _handle_new_comment(self, text, parent_comment_id=None):
        ...
        comment_repo = CommentRepository(self.driver)

        try:
            new_comment = comment_repo.add_comment_to_post(
    ...
    def _handle_like_comment(self, comment_id):
        ...
        try:
            new_likes = comment_repo.increment_comment_points(comment_id)
            # Update the comment widget's like count
    ...
    def _update_comment_likes(self, comment_id, new_likes):
        """Update the like count for a specific comment"""
        # Search through all comment widgets to find the matching one
    ...
    def set_post(self, post_data, user_id):
        """
        Sets the post content and comment section.
        :param post_data: Dictionary with all post information.
        :param user_id: ID of the current user.
        """
        self.user_id = user_id
        self.post_id = post_data["id"]

        # Remove previous FeedPostWidget if exists
        self.clear_layout(self.feed_post_layout)
        self.feed_post_widget = FeedPostWidget(post_data)
        self.feed_post_layout.addWidget(self.feed_post_widget)

        # Remove and re-add comment section with our updated implementation
        ...
        # Update the post author avatar if available
